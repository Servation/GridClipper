...
async def process_clip_queue():
    ...
    while clip_manager.queue:
        ...
        cmd = [
            ...
            "-ss", job["start_time"], "-to", job["end_time"],
            "-c", "copy", str(out_file)
        ]
...
def create_clip(req: ClipRequest, background_tasks: BackgroundTasks):
    ...
    out_file = out_dir / f"{video.stem}_clip_{req.start_time.replace(':', '')}-{req.end_time.replace(':', '')}{video.suffix}"
...
def start_ai_filter_batch(req: AIFilterRequest, background_tasks: BackgroundTasks):
    ...
    return {"status": "added", "message": f"Added {added_count} contact sheets to AI Filter queue"}
